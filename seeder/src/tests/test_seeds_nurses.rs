...
use crate::{
    seeds::{
        seeds_departments::seeds_departments, seeds_doctors::seeds_doctors,
        seeds_employees::seeds_employees, seeds_nurses::seeds_nurses,
        seeds_polyclinic::seeds_polyclinic, seeds_rooms::seeds_rooms,
    },
    tests::context::{init_db_lock, with_db_lock, TestContext},
};

#[tokio::test]
async fn test_seeds_nurse_success() {
    init_db_lock();
    ...
    with_db_lock(|| async {
        ...
        let result = seeds_nurses(&txn).await;

        assert!(result.is_ok(), "Seeding nurses failed: {:?}", result.err());

        let nurse_count = entity::nurses::Entity::find()
            .count(&txn)
            .await
            .expect("Failed to count nurses");

        assert!(nurse_count > 0, "Nurses should be seeded successfully",);
