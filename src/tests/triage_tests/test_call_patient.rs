...
use crate::{
    handlers::triage::triage_handler::triage_call_patient,
    middleware::{
...
async fn test_call() {
    ...
    with_lock(LockKind::Global, || async {
        ...
        let app = Router::new()
            .route(
                "/api/v1/triage/call/{visit_type}/{queue_number}",
        ...
        let cache_key = format!("call:queue:{visit_type}:id:1");
        ...
        let response = app
            .oneshot(
                Request::patch(format!("/api/v1/triage/call/{visit_type}/1"))
                    .header(
        ...
        assert_eq!(json["message"], "Call patient successful");
