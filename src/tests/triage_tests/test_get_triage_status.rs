...
use crate::{
    handlers::triage::triage_handler::triage_queue_status,
    middleware::{
...
#[tokio::test]
async fn test_get_triage_status() {
    ...
    with_lock(LockKind::Global, || async {
        ...
        let app = Router::new()
            .route(
                "/api/v1/triage/queue/{visit_type}/{queue_number}",
        ...
        let cache_key = format!("triage:queue:{visit_type}:id:1");
        ...
        let response = app
            .oneshot(
                Request::get(format!("/api/v1/triage/queue/{visit_type}/1"))
                    .header(
        ...
        assert_eq!(json["message"], "Get triage queue status successful");
        assert!(json["data"]["queue_number"].is_number());
