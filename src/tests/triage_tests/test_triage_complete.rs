...
use crate::{
    handlers::triage::triage_handler::triage_complete,
    middleware::{
    ...
    tests::{
        ...
        util::{admin_login, call_patient},
    },
};

#[tokio::test]
async fn test_triage_complete() {
    ...
    with_lock(LockKind::Global, || async {
        ...
        call_patient(&ctx.db, visit_type).await;
        ...
        let cache_key = format!("complete:queue:{visit_type}:id:1");
        ...
        let app = Router::new()
            .route(
                "/api/v1/triage/complete/{visit_type}/{queue_number}",
        ...
        let response = app
            .oneshot(
                Request::patch(format!("/api/v1/triage/complete/{visit_type}/1"))
                    .header(
        ...
        assert_eq!(json["message"], "Triage patient complete");
